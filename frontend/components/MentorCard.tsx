...
const MentorCard: React.FC<MentorCardProp> = ({color, topVoice}) => {
	return (
		<Card className="bg-white shadow-none min-w-[250px] w-full relative flex-1 rounded-[24px] overflow-hidden flex flex-col items-center">
			<CardHeader className="mt-16 p-0">
				<div
					className={`absolute -left-10 -right-10 top-0 h-72 ellipse`}
					style={{ backgroundColor: color }}
				></div>
				{topVoice && (
					<div className="absolute flex items-center gap-2 top-1 left-2 px-3 py-2 bg-zinc-500/20 rounded-2xl">
						<svg
						...
						<span className="text-xs font-medium text-[#4C286A]">
							Top Voice
						</span>
					</div>
				)}
